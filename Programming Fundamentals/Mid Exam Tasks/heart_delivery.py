even_integers = [int(num) for num in input().split("@")]
command = input()
length = 0
while command != "Love!":
    ...
    index = int(index)
    length += index
    if len(even_integers) > length:
        if even_integers[length] <= 0:
            print(f"Place {length} already had Valentine's day.")
        else:
            even_integers[length] -= 2
            if even_integers[length] <= 0:
...
# He will end up at index 2 and decrease the needed hearts by 2: [6, 6, 4]. Next, he jumps again with a length of 2 and goes outside the neighborhood, so he goes back to the first house (index 0) and again decreases the needed hearts there: [4, 6, 4].
# Input
